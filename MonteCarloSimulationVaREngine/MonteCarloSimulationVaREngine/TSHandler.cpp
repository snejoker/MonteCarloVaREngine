...
TSHandler::TSHandler(std::vector<std::vector<double>> riskFactorTimeSeries_, std::vector<AbsOrRel> absOrRelReturnsVector_, unsigned long daysBackUsed_) : riskFactorTimeSeries(riskFactorTimeSeries_), absOrRelReturnsVector(absOrRelReturnsVector_), daysBackUsed(daysBackUsed_)
{
	transformedToReturns = false;
}

std::vector<double> TSHandler::GetMostRecentValues(unsigned long startingDaysBack) const
{
...
std::vector<std::vector<double>> TSHandler::GetReturns() const
{
	return returns;
}
...
double TSHandler::compute_covariance(std::vector<std::vector<double>> d, int i, int j) {
	double cov = 0;
...
std::vector<std::vector<double>> TSHandler::GetCovarianceMatrix() const
{
	return covMatrix;
}

std::vector<std::vector<double>> TSHandler::GetPartsOfCovarianceMatrix(std::vector<unsigned long> diagonalIndices) const
{
